 ...
 */
export function renderLatex(tex = '', displayMode = false) {
  const cleanTex = String(tex || '').trim()
  if (!cleanTex) return ''
  if (typeof window !== 'undefined' && window.katex && typeof window.katex.renderToString === 'function') {
    try {
      return window.katex.renderToString(cleanTex, { displayMode, throwOnError: false })
    } catch (_) {}
  }
  const tag = displayMode ? 'div' : 'span'
  const cls = displayMode ? 'math-block' : 'math-inline'
  const escaped = cleanTex.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
  return `<${tag} class="${cls}" data-tex="${escaped}">${displayMode ? '$$' : '$'}${escaped}${displayMode ? '$$' : '$'}</${tag}>`
}
...
export function renderMarkdown(md) {
  ...
  const saveToken = html => {
    const key = `@@@ANON_TOKEN_${tokens.length}@@@`
    tokens.push(html)
    return key
  }
  ...
  let text = md.replace(/```(\w*)\n([\s\S]*?)```/g, (_, lang, code) => {
    ...
    return saveToken(`<pre class="md-code"><code class="language-${cleanLang}">${cleanCode.trim()}</code></pre>`)
  })

  // 2. Preserve display math $$...$$
  text = text.replace(/\$\$([\s\S]*?)\$\$/g, (_, math) => {
    return saveToken(renderLatex(math, true))
  })

  // 3. Preserve inline code `...`
  text = text.replace(/`([^`\n]+)`/g, (_, code) => {
    const clean = code.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
    return saveToken(`<code class="md-inline">${clean}</code>`)
  })

  // 4. Preserve inline math $...$ (ensure not empty, no newlines, and not pure price like $100 without math symbols or simple variable)
  text = text.replace(/(^|[^\\])\$([^\$\n]+?)\$/g, (match, prefix, math) => {
    if (/^\d+(\.\d+)?$/.test(math.trim())) return match // Skip currency numbers like $100
    return `${prefix}${saveToken(renderLatex(math, false))}`
  })

  // 5. Standard markdown escapes & syntax
  let html = text.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
  for (let level = 6; level >= 1; level--) {
    html = html.replace(new RegExp(`^${'#'.repeat(level)} (.*$)`, 'gim'), `<h${level}>$1</h${level}>`)
  }
  ...
  tokens.forEach((tokenHtml, i) => {
    html = html.split(`@@@ANON_TOKEN_${i}@@@`).join(tokenHtml)
  })
