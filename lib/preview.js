...
export function buildStandaloneHtml(content = '', options = {}) {
    ...
    <script>
      (function() {
        ...
        function serialize(a) {
          ...
          if (typeof a === 'object') {
            try { return JSON.stringify(a); } catch(e) { return Object.prototype.toString.call(a); }
          }
          return String(a);
        }
        function forward(level, args) {
          try {
            ...
            window.parent.postMessage({ type: 'preview-console', level: level, text: msg }, '*');
          } catch(e) {}
        }
        console.log = function() { forward('log', arguments); if (origLog) origLog.apply(console, arguments); };
        console.warn = function() { forward('warn', arguments); if (origWarn) origWarn.apply(console, arguments); };
        console.error = function() { forward('error', arguments); if (origErr) origErr.apply(console, arguments); };
      })();
    </script>
  `
  ...
  if (isFullDoc) {
    ...
    if (/<head>/i.test(result)) {
      ...
      return result.replace(/(<html[\s>][^>]*>)/i, `$1\n<head>\n${interceptors}\n</head>`)
    } else {
      return `${interceptors}\n${result}`
    }
  }
