...
export class VoiceMesh {
  ...
  setDeafened(deafened) {
    ...
    for (const peer of this.peers.values()) {
      try { peer.audioEl.muted = this.isDeafened } catch (_) {}
    }
  }
  ...
  createPeer(peerCid) {
    ...
    pc.onicecandidate = event => {
      if (event.candidate) this.sendSignal(id, { type: 'voice-candidate', senderCid: this.myCid, candidate: event.candidate })
    }
  ...
  async callPeer(peerCid) {
    ...
    try {
      ...
      this.sendSignal(id, { type: 'voice-offer', senderCid: this.myCid, sdp: peer.pc.localDescription })
    } catch (error) {
  ...
  async handleSignal(senderCid, signal) {
    ...
    if (!this.isActive || !id || id === this.myCid || !signal) return
    ...
    try {
      if (signal.type === 'voice-offer') {
        ...
        this.sendSignal(id, { type: 'voice-answer', senderCid: this.myCid, sdp: pc.localDescription })
