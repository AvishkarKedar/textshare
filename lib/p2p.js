...
export class P2PMesh {
  ...
  createPeer(peerCid) {
    ...
    pc.onicecandidate = event => {
      if (event.candidate) this.sendSignal(id, { type: 'candidate', candidate: event.candidate })
    }
  ...
  async connectToPeer(peerCid) {
    ...
    try {
      ...
      this.sendSignal(id, { type: 'offer', sdp: peer.pc.localDescription })
    } catch (error) {
  ...
  async handleSignal(senderCid, signal) {
    ...
    if (id === this.myCid || !signal || typeof signal.type !== 'string') return
    ...
    try {
      if (signal.type === 'offer') {
        ...
        this.sendSignal(id, { type: 'answer', sdp: pc.localDescription })
