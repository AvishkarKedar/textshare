  ...
  return new Promise(resolve => {
    ...
    const workerScript = `
      self.onmessage = function(e) {
        ...
        try {
          var rawStdin = e.data.stdin || '';
          var inputLines = rawStdin ? rawStdin.split('\n') : [];
          var prompt = function(msg) {
            if (msg !== undefined) logs.push(String(msg));
            return inputLines.length > 0 ? inputLines.shift() : '';
          };
          var readline = prompt;
          var fn = new Function('stdin', 'prompt', 'readline', e.data.code);
          var res = fn(rawStdin, prompt, readline);
...
export async function runCode({ language, code, stdin = '', relayHost = '' }) {
  ...
  // 1. In-browser client execution for JavaScript (unless using Node-specific APIs)
  const isNodeSpecific = lang === 'node' || code.includes('require(') || code.includes('process.')
  if ((lang === 'javascript' || lang === 'js') && !isNodeSpecific) {
    result = await runJsInWorker(code, stdin)
    if (!result.ok && (result.stderr.includes('require is not defined') || result.stderr.includes('process is not defined'))) {
      result = null
    }
  }

  const runLang = (lang === 'node' || lang === 'javascript' || lang === 'js') ? 'node' : (lang === 'c++' ? 'cpp' : lang)
  ...
  if (!result && relayHost) {
    try {
      ...
      const res = await fetch(`${proto}//${clean}/run`, {
        ...
        body: JSON.stringify({ language: runLang === 'javascript' ? 'node' : runLang, code, stdin }),
      })
      if (res.ok) {
  ...
  if (!result) {
    try {
      const pistonLang = runLang === 'cpp' ? 'cpp' : (runLang === 'node' ? 'javascript' : runLang)
